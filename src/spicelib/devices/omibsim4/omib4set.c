...
int
omiBSIM4setup(SMPmatrix* matrix,
              GENmodel* inModel,
              CKTcircuit* ckt,
              int* states)
{
  ...
  for (job = ((TSKtask*)ft_curckt->ci_curTask)->jobs; job;
       job = job->JOBnextJob) {
  ...
  for (; model != NULL; model = omiBSIM4nextModel(
                          model)) { /* process defaults of model parameters */
    ...
    else if ((model->omiBSIM4rbodyMod != 0) && (model->omiBSIM4rbodyMod != 1) &&
             (model->omiBSIM4rbodyMod != 2)) {
      model->omiBSIM4rbodyMod = 0;
    ...
    if (!model->omiBSIM4tnoiModGiven)
      model->omiBSIM4tnoiMod =
        0; /* WDLiu: tnoiMod=1 needs to set internal S/D nodes */
    else if ((model->omiBSIM4tnoiMod != 0) && (model->omiBSIM4tnoiMod != 1) &&
             (model->omiBSIM4tnoiMod != 2)) /* v4.7 */
    {
    ...
    else if ((model->omiBSIM4mtrlCompatMod != 0) &&
             (model->omiBSIM4mtrlCompatMod != 1)) {
      model->omiBSIM4mtrlCompatMod = 0;
    ...
    if (!model->omiBSIM4uaGiven)
      model->omiBSIM4ua =
        ((model->omiBSIM4mobMod == 2)) ? 1.0e-15 : 1.0e-9; /* unit m/V */
    ...
    if (!model->omiBSIM4aigsdGiven &&
        (model->omiBSIM4aigsGiven || model->omiBSIM4aigdGiven)) {
    ...
    if (!model->omiBSIM4bigsdGiven &&
        (model->omiBSIM4bigsGiven || model->omiBSIM4bigdGiven)) {
    ...
    if (!model->omiBSIM4cigsdGiven &&
        (model->omiBSIM4cigsGiven || model->omiBSIM4cigdGiven)) {
    ...
    if (!model->omiBSIM4DunitLengthSidewallJctCapGiven)
      model->omiBSIM4DunitLengthSidewallJctCap =
        model->omiBSIM4SunitLengthSidewallJctCap;
    if (!model->omiBSIM4SunitLengthGateSidewallJctCapGiven)
      model->omiBSIM4SunitLengthGateSidewallJctCap =
        model->omiBSIM4SunitLengthSidewallJctCap;
    if (!model->omiBSIM4DunitLengthGateSidewallJctCapGiven)
      model->omiBSIM4DunitLengthGateSidewallJctCap =
        model->omiBSIM4SunitLengthGateSidewallJctCap;
    ...
    if (!model->omiBSIM4DsidewallJctPotentialGiven)
      model->omiBSIM4DsidewallJctPotential =
        model->omiBSIM4SsidewallJctPotential;
    if (!model->omiBSIM4SGatesidewallJctPotentialGiven)
      model->omiBSIM4SGatesidewallJctPotential =
        model->omiBSIM4SsidewallJctPotential;
    if (!model->omiBSIM4DGatesidewallJctPotentialGiven)
      model->omiBSIM4DGatesidewallJctPotential =
        model->omiBSIM4SGatesidewallJctPotential;
    ...
    for (here = omiBSIM4instances(model); here != NULL;
      ...
      *states += omiBSIM4numStates;
      ...
      else if ((here->omiBSIM4rbodyMod != 0) && (here->omiBSIM4rbodyMod != 1) &&
               (here->omiBSIM4rbodyMod != 2)) {
        here->omiBSIM4rbodyMod = model->omiBSIM4rbodyMod;
      ...
      if ((model->omiBSIM4rdsMod != 0) ||
        ...
        if (here->omiBSIM4drainSquaresGiven && here->omiBSIM4drainSquares > 0) {
          createNode = 1;
        } else if (!here->omiBSIM4drainSquaresGiven &&
                   (here->omiBSIM4rgeoMod != 0)) {
          omiBSIM4RdseffGeo(here->omiBSIM4nf * here->omiBSIM4m,
                            here->omiBSIM4geoMod,
                            here->omiBSIM4rgeoMod,
                            here->omiBSIM4min,
                            here->omiBSIM4w,
                            model->omiBSIM4sheetResistance,
                            DMCGeff,
                            DMCIeff,
                            DMDGeff,
                            0,
...
int
omiBSIM4unsetup(GENmodel* inModel, CKTcircuit* ckt)
{
#ifndef HAS_BATCHSIM
  omiBSIM4model* model;
  omiBSIM4instance* here;
